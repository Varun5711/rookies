...
export class AnalyticsService {
  ...
  constructor(
    ...
    this.authSvcUrl = this.configService.get(
      'AUTH_SVC_URL',
      'http://localhost:3001/api',
    );
    this.healthcareSvcUrl = this.configService.get(
      'HEALTHCARE_SVC_URL',
      'http://localhost:3010/api',
    );
    this.agricultureSvcUrl = this.configService.get(
      'AGRICULTURE_SVC_URL',
      'http://localhost:3011/api',
    );
    this.urbanSvcUrl = this.configService.get(
      'URBAN_SVC_URL',
      'http://localhost:3012/api',
    );
    this.auditSvcUrl = this.configService.get(
      'AUDIT_SVC_URL',
  ...
  async getDashboardStats(token: string) {
    try {
      ...
      const [usersData, appointmentsData, applicationsData, grievancesData] =
        await Promise.allSettled([
          this.fetchWithFallback(`${this.authSvcUrl}/users/count`, headers, 0),
          this.fetchWithFallback(
            `${this.healthcareSvcUrl}/admin/appointments/stats`,
            headers,
            [],
          ),
          this.fetchWithFallback(
            `${this.agricultureSvcUrl}/admin/schemes/stats`,
            headers,
            {},
          ),
          this.fetchWithFallback(
            `${this.urbanSvcUrl}/admin/grievances/stats`,
            headers,
            {},
          ),
        ]);
      ...
      const appointmentStats =
        appointmentsData.status === 'fulfilled' ? appointmentsData.value : [];
      const applicationStats =
        applicationsData.status === 'fulfilled' ? applicationsData.value : {};
      const grievanceStats =
        grievancesData.status === 'fulfilled' ? grievancesData.value : {};
