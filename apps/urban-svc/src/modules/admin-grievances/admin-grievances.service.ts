...
import {
  Grievance,
  GrievanceStatus,
} from '../grievances/entities/grievance.entity';
...
export class AdminGrievancesService {
  ...
  async findAll(page = 1, limit = 10, status?: GrievanceStatus, ward?: string) {
    const query = this.grievanceRepository
  ...
  async assign(id: string, dto: AssignGrievanceDto): Promise<Grievance> {
    ...
    if (dto.assignedDepartment)
      grievance.assignedDepartment = dto.assignedDepartment;
  ...
  async getStats() {
    const total = await this.grievanceRepository.count();

    const byStatus = await this.grievanceRepository
      .createQueryBuilder('grievance')
    ...
    const byPriority = await this.grievanceRepository
      .createQueryBuilder('grievance')
      .select('grievance.priority', 'priority')
      .addSelect('COUNT(*)', 'count')
      .groupBy('grievance.priority')
      .getRawMany();
